...
class MainWindow(Gtk.Window):
    ...
    def __init__(self):
        ...
        # setting variables
        self.control = 0  # 0:stop,1:run/continue,2:cancel previous task and run new task,3: finish previous task and reset motor
        ...
        self.dist2vol = 30/83.0 #for 30mL syringe
        self.pre_dist = 0
    ...
    def prepare_task(self):
        if self.remain_time > 0 and self.mode != 1:
            #have a pop up to ask continue or restart, only for infuse
    ...
    def display_vol(self):
        good = self.ser.read()
        #print(good)
        # get info about the finished distance in um
        if good == b'A':
            num = int(self.ser.read())
            dist = int(self.ser.read(num))
            #print(dist)
            if dist == self.pre_dist:
                self.finished_volume = int(dist * self.dist2vol)
                #need better strtage to show this message
    ...
    def display_time(self):
        ...
        if self.control == 1 or self.control == 2:
            if self.remain_time > 0:
                ...
                #might also send command about finishing task to the motor(the whole system?) and reset the interface
    ...
    def startclocktimer(self):
        ...
        GObject.timeout_add(1000, self.display_vol)
